...
class CreateDemoAccount extends Command
{
    ...
    public function handle(): int
    {
        ...
        $supplied = (string) $this->option('password');

        if ($supplied !== '' && mb_strlen($supplied) < self::MIN_PASSWORD_LENGTH) {
            $this->error('--password must be at least '.self::MIN_PASSWORD_LENGTH.' characters.');

            return self::FAILURE;
        }

        // Generated by default and shown once, so no known credential sits in
        // the repo, the deploy config, or the shell history of whoever ran
        // this. --password exists for the hosts where that does not work:
        // some platforms run a deploy pre-command in a container whose stdout
        // never reaches the log, and a password printed there is simply lost.
        $password = $supplied !== '' ? $supplied : Str::password(20, symbols: false);
        ...
        if ($supplied !== '') {
            $this->comment('The password came from --password, so it is recorded wherever that command line is stored. Re-run without the option to replace it with a generated one.');
        } else {
